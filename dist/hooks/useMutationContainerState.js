...
const useMutationContainerState = (useMutation) => {
    return useMutation(({ storage }, nodeId, containerState) => {
        storage.get("nodeMap").get(nodeId)?.get("state").get("containerState").update(Object.fromEntries(Object.entries(containerState).map(([key, value]) => [key, Math.round(value)])));
    }, []);
};
